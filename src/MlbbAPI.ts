import type {HeroRankingOptions} from "./types/hero-ranking";
import type {HeroPositionOptions} from "./types/hero-position";
import {Languages, MakeRequest, MlbbAPIOptions} from "./types/_core";
import {HeroRateDays} from "./enums/_enums";
import {createMlbbFetcher} from "./cache/cache-fetcher";
import {CacheOptions, CacheStore, CreateFetcherOptions} from "./types/_cache";
...
export class MlbbAPI {
    ...
    public readonly makeRequest: MakeRequest;

    constructor(options: Partial<MlbbAPIOptions> = {}) {
        const baseURL = options.baseURL ?? "https://mlbb-stats.ridwaanhall.com/";
        const language = options.language ?? Languages.English;

        this.baseURL = baseURL;

        const fetcherOpts: Partial<CreateFetcherOptions> = {
            baseURL,
            language,
            ttlOverrides: options.cache?.ttlOverrides,
            enabled: options.cache?.enabled ?? true,
            defaultTTL: options.cache?.defaultTTL ?? 300_000,
            memoryMaxEntries: options.cache?.memoryMaxEntries ?? 500,
            store: options.cache?.store,
        };

        this.makeRequest = createMlbbFetcher(fetcherOpts);
    }
     ...
     */
    async getHeroRanking(options: Partial<HeroRankingOptions> = {}, cache?: CacheOptions) {
        return this.makeRequest("/api/hero-rank", { query: options }, cache);
    }
     ...
     */
    async getHeroPosition(options: Partial<HeroPositionOptions> = {}, cache?: CacheOptions) {
        return this.makeRequest("/api/hero-position", { query: options }, cache);
    }
     ...
     */
    async getHeroDetail(heroId: number, cache?: CacheOptions) {
        return this.makeRequest(`/api/hero-detail/${heroId}/`, undefined, cache);
    }
     ...
     */
    async getHeroDetailStats(heroId: number, cache?: CacheOptions) {
        return this.makeRequest(`/api/hero-detail-stats/${heroId}/`, undefined, cache);
    }
     ...
     */
    async getHeroSkillCombo(heroId: number, cache?: CacheOptions) {
        return this.makeRequest(`/api/hero-skill-combo/${heroId}/`, undefined, cache);
    }

    /**
     * @description This API endpoint uses the main_heroid path parameter (1-129) to retrieve counter information for a hero.
     */
    async getHeroCounter(heroId: number, cache?: CacheOptions) {
        return this.makeRequest(`/api/hero-counter/${heroId}/`, undefined, cache);
    }

    /**
     * @description API endpoint uses main_heroid (1-129) to fetch hero compatibility data.
     */
    async getHeroCompatibility(heroId: number, cache?: CacheOptions) {
        return this.makeRequest(`/api/hero-compatibility/${heroId}/`, undefined, cache);
    }
     ...
     */
    async getHeroRate(heroId: number, pastDays: HeroRateDays, cache?: CacheOptions) {
        return this.makeRequest(`/api/hero-rate/${heroId}/`, { query: { "past-days": pastDays } }, cache);
    }
     ...
     */
    async getHeroRelation(heroId: number, cache?: CacheOptions) {
        return this.makeRequest(`/api/hero-relation/${heroId}/`, undefined, cache);
    }
